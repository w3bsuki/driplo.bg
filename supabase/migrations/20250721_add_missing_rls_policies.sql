...
CREATE POLICY "Users can view messages for their disputes" ON dispute_messages
    FOR SELECT USING (
        auth.uid() IN (
            SELECT buyer_id FROM disputes WHERE id = dispute_messages.dispute_id
            UNION
            SELECT seller_id FROM disputes WHERE id = dispute_messages.dispute_id
        )
    );
...
CREATE POLICY "Users can send messages to their disputes" ON dispute_messages
    FOR INSERT WITH CHECK (
        ...
        auth.uid() IN (
            SELECT buyer_id FROM disputes WHERE id = dispute_id
            UNION
            SELECT seller_id FROM disputes WHERE id = dispute_id
        )
    );
